...
begin
  ...
  select coalesce(is_admin, false), coalesce(is_super_admin, false)
  ...
  select coalesce(strike_threshold, 3)
    into v_threshold
  ...
  set reservation_strikes = v_next_strikes,
      reservation_restricted_until = case
        ...
        else reservation_restricted_until
      end
...
revoke all on function public.admin_set_user_reservation_strikes(uuid, integer) from public;
revoke all on function public.admin_set_user_reservation_strikes(uuid, integer) from anon;
grant execute on function public.admin_set_user_reservation_strikes(uuid, integer) to authenticated;
